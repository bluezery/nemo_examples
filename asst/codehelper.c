...
uint64_t crc32_from_string(const char *str)
{
	const unsigned char *ustr;
	uint32_t value = 0;

	for (ustr = (const unsigned char *)str; *ustr != '\0'; ustr++) {
		value = crc32_table[(value ^ *ustr) & 0xff] ^ (value >> 8);
	}
...
int base64_encode(unsigned char *input, int length, unsigned char *output)
{
	char buff1[3];
	char buff2[4];
	unsigned char i = 0, j;
	...
	while (input_cnt < length) {
		buff1[i++] = input[input_cnt++];
		if (i == 3) {
			output[output_cnt++] = base64_encode_table[(buff1[0] & 0xfc) >> 2];
			output[output_cnt++] = base64_encode_table[((buff1[0] & 0x03) << 4) + ((buff1[1] & 0xf0) >> 4)];
			output[output_cnt++] = base64_encode_table[((buff1[1] & 0x0f) << 2) + ((buff1[2] & 0xc0) >> 6)];
			output[output_cnt++] = base64_encode_table[buff1[2] & 0x3f];
			i=0;
		}
	}
	...
	if (i) {
		...
		for (j = 0; j < (i + 1); j++) {
			output[output_cnt++] = base64_encode_table[buff2[j]];
		}
		while (i++ < 3) {
			output[output_cnt++] = '=';
		}
...
int base64_decode(unsigned char *input, int length, unsigned char *output)
{
	...
	while (input_cnt < length) {
		...
		if (++i == 4) {
			for (i = 0; i != 4; i++)
				buff2[i] = base64_decode_table[buff2[i]];
	...
	if (i) {
		for (j = i; j < 4; j++)
			buff2[j] = '\0';
		for (j = 0; j < 4; j++)
			buff2[j] = base64_decode_table[buff2[j]];
