...
static void nemotale_update_uniforms(struct nemotale *tale)
{
	struct nemogltale *context = (struct nemogltale *)tale->glcontext;
	struct glshader *shader = context->current_shader;
...
static void nemotale_repaint_node(struct nemotale *tale, struct talenode *node, pixman_region32_t *damage)
{
	...
	if (pixman_region32_not_empty(&repaint)) {
		...
		if (pixman_region32_not_empty(&node->opaque)) {
			nemotale_use_shader(tale, &context->texture_shader_rgbx);
			nemotale_update_uniforms(tale);
...
int nemotale_composite_egl(struct nemotale *tale)
{
	...
	if (tale->type & NEMOTALE_COMPOSITE_TYPE_CLEAR) {
